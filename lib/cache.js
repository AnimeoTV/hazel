...
module.exports = class Cache {
  ...
  async cacheYmlReleases(url) {
    ...
    let content = await convertStream(body)
    const matches = content.match(/url:.+(exe|AppImage|dmg)/gim)

    if (matches.length === 0) {
      throw new Error(
        `Tried to cache electron-updater .yml file, but failed. electron-updater .yml file content doesn't contain nupkg`
      )
    }

    const fileName = matches[0].replace("url: ", "")

    content = content.replace(
      matches[0],
      `url: ${this.config.url}/download/latest/${fileName}`
    )
    console.log('content', content)
