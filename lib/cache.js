...
module.exports = class Cache {
  ...
  async cacheYmlReleases(url) {
    ...
    let content = await convertStream(body)
    const matches = content.match(/[^ ]*\.exe|AppImage|dmg/gim)

    if (matches.length === 0) {
      throw new Error(
        `Tried to cache electron-updater .yml file, but failed. RELEASES content doesn't contain nupkg`
      )
    }
  ...
  async refreshCache() {
    ...
    this.latest.files = {}
    this.latest.files.electronUpdate = {}
    ...
    for (const asset of release.assets) {
      ...
      if (name.match(/latest(-mac|-linux)*\.yml/gim)) {
        try {
          this.latest.files.electronUpdate[name] = await this.cacheYmlReleases(url)
        } catch (err) {
          console.error(err)
        }
        continue
