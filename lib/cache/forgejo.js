...
module.exports = class ForgejoCache extends Cache {
  ...
  async refreshCache() {
    const { account, repository, pre, token, instance_url } = this.config
    const repo = account + '/' + repository
    const url = `${instance_url}/api/v1/repos/${repo}/releases`
    ...
    for (const asset of release.assets) {
      const { name, browser_download_url, content_type, size } = asset

      if (name === 'RELEASES') {
        try {
          this.latest.files.RELEASES = await this.cacheReleaseList(
            `${instance_url}/attachments/${asset.uuid}`
          )
        } catch (err) {
          console.error(err)
        }
      ...
      const entry = {
        name,
        api_url: `${instance_url}/attachments/${asset.uuid}`,
