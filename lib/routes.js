...
const GitHubProxyDownload = require('./proxy/github')
const ForgejoProxyDownload = require('./proxy/forgejo')

module.exports = ({ cache, config }) => {
  const { loadCache } = cache
  const exports = {}
  const { token, url, repo_platform } = config
  ...
  exports.download = async (req, res) => {
    ...
    if (shouldProxyPrivateDownload) {
      switch (repo_platform) {
        case 'github':
          GitHubProxyDownload(platforms[platform], req, res, token)
          break
        case 'forgejo':
          ForgejoProxyDownload(platforms[platform], req, res, token)
          break
      }
      return
    }
  ...
  exports.downloadPlatform = async (req, res) => {
    ...
    if (token && typeof token === 'string' && token.length > 0) {
      switch (repo_platform) {
        case 'github':
          GitHubProxyDownload(latest.platforms[platform], req, res, token)
          break
        case 'forgejo':
          ForgejoProxyDownload(latest.platforms[platform], req, res, token)
          break
      }
      return
    }
  ...
  exports.downloadLatest = async (req, res) => {
    ...
    if (token && typeof token === 'string' && token.length > 0) {
      switch (repo_platform) {
        case 'github':
          GitHubProxyDownload(latest.files[file], req, res, token)
          break
        case 'forgejo':
          ForgejoProxyDownload(latest.files[file], req, res, token)
          break
      }
      return
    }
  ...
  exports.overview = async (req, res) => {
    ...
    try {
      ...
      const details = {
        ...
        date: formatDistanceToNow(latest.pub_date, {
          addSuffix: true,
        }),
        files: latest.platforms,
        version: latest.version,
        releaseNotes: `${config.repo_platform == 'forgejo' ? config.instance_url : 'https://github.com'}/${config.account}/${
          config.repository
        }/releases/tag/${latest.version}`,
        allReleases: `${config.repo_platform == 'forgejo' ? config.instance_url : 'https://github.com'}/${config.account}/${
          config.repository
        }/releases`,
        git: `${config.repo_platform == 'forgejo' ? config.instance_url : 'https://github.com'}/${config.account}/${config.repository}`,
      }
